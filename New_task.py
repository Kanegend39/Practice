...
def phase_recovery(E0, lambda0, left, right, step_lambda, step, delta_lambda, delta_l2, b, gamma):
    ...
    b = 2 * lambda0 * 1e-2
    # print(b)
    delta_l2_new = delta_l2 + b * np.cos(gamma * delta_l1)
    ...
    for i in range(len(range_lambda)):
        ...
        I += E * (4 * (np.cos(k[i] * delta_l1) + np.cos(k[i] * delta_l2_new)) + 2 * (np.cos(
            k[i] * (delta_l1 + delta_l2_new)) + np.cos(k[i] * (delta_l1 - delta_l2_new))))
    ...
    phi = np.unwrap(comp, math.pi) - (delta_l1 * approximated_line[0] + approximated_line[1])
    #plt.plot(delta_l1, phi)  # фиолетовый
    # print(np.unwrap(comp, math.pi) - (delta_l1 * approximated_line[0] + approximated_line[1]))
    #plt.show()
    return delta_l1, I, phi, a
